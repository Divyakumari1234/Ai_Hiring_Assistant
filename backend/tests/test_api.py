...
def test_health_and_search():
    with TestClient(app) as client:
        ...
        response = client.post("/api/search", json={"job_description":"Senior product designer with Figma and user research experience", "location":"India", "provider":"demo"})
...
def test_demo_outreach_and_call_dashboard():
    with TestClient(app) as client:
        response = client.post("/api/outreach", json={"candidate_ids":["c1"], "channel":"voice"})
        assert response.status_code == 200
...
def test_hunar_webhook_updates_structured_result():
    with TestClient(app) as client:
        created = client.post("/api/outreach", json={"candidate_ids":["c2"]}).json()["calls"][0]
        result = {"interest":"High", "notice_period":"30 days"}
        response = client.post("/api/webhooks/hunar", json={"call_id":created["id"], "status":"COMPLETED", "duration_seconds":120, "result":result})
        assert response.status_code == 200
